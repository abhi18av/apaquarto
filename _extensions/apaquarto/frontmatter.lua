    ...
    Pandoc = function(doc)
      ...
      if authornote then
        ...
        if authornote.disclosures then
          ...
          third_paragraph = extend_paragraph(third_paragraph, authornote.disclosures["authorship-agreements"] or authornote["authorship-agreements"] or meta["authorship-agreements"], pandoc.LineBreak())
        ...
        if #credit_paragraph.content > 1 then
          ...
          if not mask and not meta["suppress-credit-statement"] then
            if FORMAT:match 'typst' then
              ...
              local author_roles_header = pandoc.Header(1, "AUTHOR ROLES AND CONTRIBUTIONS")
              body:extend({author_roles_header})
            end
      ...
      if meta.apaabstract and #meta.apaabstract > 0 and not meta["suppress-abstract"] then
        local abstractheadertext = "Abstract"
        if meta.language and meta.language["section-title-abstract"] then
          abstractheadertext = pandoc.utils.stringify(meta.language["section-title-abstract"]):upper()
        end
